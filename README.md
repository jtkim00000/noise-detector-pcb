...
The following documentation is a full explaination and walk through of my Noise Detector PCB, an analog circuit. 

The circuit includes an analog signal chain (amplification → peak detection → timed delay → threshold comparison). The components include a condesnser microphone, LM358 op-amp, LM311 comparator, and a Schmitt Trigger Inverter. 
...
All components in this circuit require a +9V, GND, and -9V. This is created by wiring two 9V batteries in series, creating an 18V different between the two, and setting the GND to the point inbetween the batteries, creating +9V, 0V, and -9V. 

### Microphone and Amplifier

![mic-amp](docs/mic-amp.png)

The Condenser Microphone simply convers sound into an electric signal. Sound comes into the microphone and vibrates a diagram which creates an induced current. This current is then used to compute a voltage which is the signal we are measuring. 

However, for this circuit component the voltage signal amplitude is far too small to read, thus we need to amplify the signal. For this I used an inverting amplified based on an LM358 OpAmp. The amplification is defined as R2/R1, where R2 is the top resistor. This is also known as the gain, in which I chose to use a gain of 100k/47 = 2127.66.  

### Peak Detector

![peak-detector](docs/peak-detector.png)

The peak detector module is designed to capture the maximum amplitude of voltage input from a time-varying signal. It does this by using an LM358 OpAmp, which when applying nodal analysis, we can considered the + and - inputs to have the same voltage. In this way wiring the output of the OpAmp to the input not used for the voltage signal creates a voltage follower circuit. 

We can extend the design of the voltage follower input by adding a capacitor and diode such that the capacitor will charge up to match the maximum voltage amplitude, but has no route to drain on its own. We add an NMOS switch to drain the capacitor, acting as a reset. 
...
The time delay module provides a controlled delay signal using an RC circuit. This provides our circuit with a fixed time that I use to control how long the LEDs light up for after the initial signal. 

The time delay first uses a comparator that compares the input signal to a minimum voltage. If the signal exceeds that threshold, the time delay begins. 
...
The output of this time delay is fed into a schmitt trigger inverter, which outputs high while the capacitor voltage is below a threshold. I fed this into another inverter, such that the peak detector resets the peak detector once the RC is above that threshold. 
